...
, prg_ram(0x2000)
{
  ...
  this->banks.prg.len = this->rom_file.rom.prg.len / 0x2000;
  ...
  if (this->rom_file.rom.chr.len != 0) {
    ...
    this->banks.chr.len = this->rom_file.rom.chr.len / 0x400;
...
Mirroring::Type Mapper_004::mirroring() const {
  if (this->rom_file.meta.mirror_mode == Mirroring::FourScreen)
